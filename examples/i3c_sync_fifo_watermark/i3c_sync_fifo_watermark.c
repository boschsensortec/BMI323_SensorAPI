    ...
    if (rslt == BMI323_OK)
    {
        ...
        if (rslt == BMI323_OK)
        {
            ...
            if (rslt == BMI323_OK)
            {
                ...
                bmi323_error_codes_print_result("bmi323_set_i3c_tc_sync_tph", rslt);

                /* Set the delay time of i3c sync */
                rslt = bmi323_set_i3c_tc_sync_tu(delay_time, &dev);
                bmi323_error_codes_print_result("bmi323_set_i3c_tc_sync_tu", rslt);

                /* Set i3c sync ODR */
                rslt = bmi323_set_i3c_tc_sync_odr(odr, &dev);
                bmi323_error_codes_print_result("bmi323_set_i3c_tc_sync_odr", rslt);

                /* Enable the i3c sync filter */
                rslt = bmi323_set_i3c_sync_filter_en(filter_en, &dev);
                bmi323_error_codes_print_result("bmi323_set_i3c_sync_filter_en", rslt);
