                ...
                if (rslt == BMI323_OK)
                {
                    ...
                    do
                    {
                        ...
                        if (int_status & BMI323_INT_STATUS_TAP)
                        {
                            ...
                            rslt = bmi323_get_regs(BMI323_REG_FEATURE_ENGINE_EVENT_STATUS, data, 2, &dev);

                            if (data[0] & BMI323_TAP_DET_STATUS_SINGLE)
                            {
                                printf("Single tap asserted\n");
                                loop--;
                            }

                            if (data[0] & BMI323_TAP_DET_STATUS_DOUBLE)
                            {
                                printf("Double tap asserted\n");
                                loop--;
                            }

                            if (data[0] & BMI323_TAP_DET_STATUS_TRIPLE)
                            {
...
static int8_t set_feature_config(struct bmi323_dev *dev)
{
    ...
    if (rslt == BMI323_OK)
    {
        ...
        if (rslt == BMI323_OK)
        {
            ...
            printf("Tap hearable configurations\n");
