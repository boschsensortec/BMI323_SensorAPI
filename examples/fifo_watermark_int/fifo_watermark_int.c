...
int main(void)
{
    ...
    uint16_t fifo_watermark_level = 2016;
     ...
     */
    struct bmi323_fifo_sens_axes_data fifo_accel_data[266];
     ...
     */
    struct bmi323_fifo_sens_axes_data fifo_gyro_data[266];
     ...
     */
    struct bmi323_fifo_temperature_data fifo_temp_data[266];
     ...
     * Calculation:
     * fifo_watermark_level = 1600, accel_frame_len = 6, gyro_frame_len = 6, temp_frame_len = 2, sensor_frame_len = 2
     * fifo_accel_frame_count = (1600 / (6+6+2+2)) = 100 frames
     */
    uint16_t fifo_accel_frame_length = 100;
     ...
     * fifo_accel_frame_count = (1600 / (6+6+2+2)) = 100 frames
     */
    uint16_t fifo_gyro_frame_length = 100;

    /* Number of temperature frames to be extracted from FIFO
     * Calculation:
     * fifo_watermark_level = 1600, accel_frame_len = 6, gyro_frame_len = 6, temp_frame_len = 2, sensor_frame_len = 2
     * fifo_temp_frame_count = (1600 / (6+6+2+2)) = 100 frames
     */
    uint16_t fifo_temp_frame_length = 100;
    ...
    bmi323_error_codes_print_result("bmi323_init\n", rslt);
    ...
    bmi323_error_codes_print_result("bmi323_set_fifo_wm", rslt);

    rslt = bmi323_get_fifo_wm(&watermark, &dev);
    bmi323_error_codes_print_result("bmi323_get_fifo_wm", rslt);
    ...
    while (count <= 3)
    {
        ...
        if ((rslt == BMI323_OK) && (int_status & BMI323_INT_STATUS_FWM))
        {
            printf("\nIteration :%d", count);
            ...
            if (rslt == BMI323_OK)
            {
                ...
                printf("\nRequested temperature data frames before parsing: %d", fifo_temp_frame_length);
